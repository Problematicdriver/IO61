...
// io61.cc
//    YOUR CODE HERE!
#define REGION_SIZ 16

inline __attribute__((always_inline)) size_t round_up(off_t n, off_t mod) {
    return n + (n % mod == 0 ? 0 : mod - n % mod);
}
...
struct io61_file {
    ...
    region_lock* reg;	// fine-grained locks for every region of the file
};
...
io61_file* io61_fdopen(int fd, int mode) {
    ...
    assert(fsize >= 0);
    int nregs = round_up(fsize, REGION_SIZ) / REGION_SIZ;
    f->reg = new region_lock[nregs];
...
int file_region(off_t pos) {
    return pos / REGION_SIZ;
}
...
int io61_lock(io61_file* f, off_t start, off_t len, int locktype) {
    ...
    // acquire lock, blocks until lock can be acquired
    std::unique_lock guard(f->m);
    while (may_overlap_with_other_lock(f, start, len)) {
        f->cv.wait(guard);
    }
    for (int ri = file_region(start); ri <= file_region(start + len - 1); ++ri) {
	// printf("thread %d lock   %d\n", std::this_thread::get_id(), ri);
        ++f->reg[ri].locked;
        f->reg[ri].owner = std::this_thread::get_id();
    }
    return 0;
...
int io61_unlock(io61_file* f, off_t start, off_t len) {
    ...
    std::unique_lock guard(f->m);
    // printf("thread %d unlock %d\n", std::this_thread::get_id(), file_region(start));
    for (int ri = file_region(start); ri <= file_region(start + len - 1); ++ri) {
        --f->reg[ri].locked;
    }
    f->cv.notify_all();
